...
def worker_task_NAG(arg):
    return nesterov_optimizer(*arg)

def worker_task_GD(arg):
    return descente_gradient(*arg)
...
if __name__ == '__main__':
    ...
    for X_train, y_train in zip(real_datasets[0], real_datasets[1]):
        ...
        # Utiliser les indices pour l'axe des x
        indices = range(len(liste_lignes_dataset))
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            results_NAG = list(executor.map(worker_task_NAG, tasks))
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            results_GD = list(executor.map(worker_task_GD, tasks))
